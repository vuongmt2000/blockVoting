...
const NewPoll = (props) => {
  const candidateName1 = useRef(null);
  const candidateName2 = useRef(null);

  const candidateName1URL = useRef(null);
  const candidateName2URL = useRef(null);
  ...
  const sendToBlockChain = async () => {
    if(candidateName1URL && candidateName2URL && candidateName1 && candidateName2 &&promptRef){
      changeDisable(true);
